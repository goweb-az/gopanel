$(document).ready(function () {
    ...
    $('#filter-user').select2({
        ...
        ajax: {
            url: function () {
                var base = '/gopanel/file-logs/users';
                var companyId = $('#filter-company').val();
    ...
    // Şirkət dəyişəndə user select-i sıfırla
    ...
    $(document).on('click', '.view-btn', function (e) {
        ...
        $.get(url, function (html) {
            ...
            // JSON datalarını render etmək üçün blade-dən window-a oturulan dəyərlərə baxırıq
            var contextData = window.logContextData || {};
            var detailsData = window.logDetailsData || {};

            if (Object.keys(contextData).length > 0) {
                $('#json-context').jsonViewer(contextData, { collapsed: true, withQuotes: true, withLinks: true });
            } else {
                $('#json-context').html('<span class="text-muted">Boş</span>');
            }

            if (Object.keys(detailsData).length > 0) {
                $('#json-details').jsonViewer(detailsData, { collapsed: true, withQuotes: true, withLinks: true });
            } else {
                $('#json-details').html('<span class="text-muted">Boş</span>');
            }
    ...
    window.copyJsonData = function(type) {
        var data = type === 'context' ? window.logContextData : window.logDetailsData;
